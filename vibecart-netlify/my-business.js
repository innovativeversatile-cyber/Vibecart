(function () {
  ...
  function renderKpis(listings, services, bookings) {
    ...
    node.innerHTML =
      ...
      '<article class="vc-biz-card"><span class="vc-pill">Bakery work cards</span><h3>' + Number((services || []).length) + '</h3><p class="note">Active cards: ' + activeBakery + "</p></article>" +
  ...
  function readBakeryForm() {
    return {
      ...
      styleTheme: document.getElementById("bakeryStyleTheme").value,
  ...
  function renderServices(services) {
    ...
    root.innerHTML = services.map(function (s) {
      return '<article class="vc-work-item" data-service-id="' + Number(s.id) + '">' +
        ...
        (s.imageUrl ? '<img src="' + escapeHtml(s.imageUrl) + '" alt="Bakery work sample" loading="lazy" />' : "") +
  ...
  function loadAll() {
    setStatus("Loading hustle dashboard...");
    return Promise.all([
