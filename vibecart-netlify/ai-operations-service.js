...
const AI_OPS_ENUM_TYPES = new Set(["marketing", "security", "inventory", "research", "product_update", "compliance", "pricing"]);

function normalizeAiOperationTypeEnum(raw) {
  const t = String(raw || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "_");
  if (AI_OPS_ENUM_TYPES.has(t)) {
    return t;
  }
...
async function createAiOperation(pool, input) {
  const operationType = normalizeAiOperationTypeEnum(input.operationType);
  const summaryText = String(input.summaryText || "").trim();
  let recommendationText = String(input.recommendationText || "").trim();
  const regNote = String(input.regulatoryNotice || "").trim();
  if (regNote) {
    recommendationText = `${recommendationText}\n\n[Compliance note] ${regNote.slice(0, 450)}`.trim();
  }
  const riskLevel = String(input.riskLevel || "medium").trim().toLowerCase();
  const rawMode = String(input.executionMode || "recommend_only").trim().toLowerCase();
  const allowedModes = new Set(["recommend_only", "owner_approval_required", "autonomous_safe"]);
  let executionMode = allowedModes.has(rawMode) ? rawMode : "owner_approval_required";
  if (executionMode === "autonomous_safe") {
  ...
  const items = [
    {
      ...
      regulatoryNotice: "Owner must validate campaigns under local marketing and consumer law before spend."
    },
    {
      ...
      riskLevel: "high",
      executionMode: "owner_approval_required",
      regulatoryNotice: "Security controls may implicate data protection duties — confirm with counsel."
    },
    {
      ...
      riskLevel: "medium",
      executionMode: "recommend_only",
      regulatoryNotice: "Cross-border listings must follow export/import and product compliance for target markets."
    }
  ];
