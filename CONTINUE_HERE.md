...
- **Smoke (API + static links + sample checkouts):** `npm run smoke:full` — hits Railway public routes, Netlify `*.netlify.app` HTML HEAD, local `deploy-web` link integrity; optional `SMOKE_MUTATIONS=0` to skip DB writes.
